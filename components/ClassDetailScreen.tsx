    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t('classLabel')} {title}</h1>
          {metaPinned && <span className="text-xs text-gray-500 dark:text-gray-400">({t('pinned')})</span>}
        </div>
        <div className="flex items-center gap-2">
          <button className="px-3 py-2 bg-gray-200 dark:bg-gray-700 rounded dark:text-white" onClick={() => setShowEdit(true)}>{t('edit')}</button>
          <button className="px-3 py-2 bg-gray-200 dark:bg-gray-700 rounded dark:text-white" onClick={() => exportClassReportPDF({ className: title, children, obs7d, media7d, activity: activityDays, aiSummary, aiSuggestions })}>{t('pdfDownload')}</button>
          <button className="px-3 py-2 bg-gray-200 dark:bg-gray-700 rounded dark:text-white" onClick={() => navigate('classes')}>{t('backToClasses')}</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div className="bg-white dark:bg-[#1a1a2e] rounded-lg shadow p-4 transition-colors">
          <div className="text-sm text-gray-500 dark:text-gray-400">{t('childrenList')}</div>
          <div className="text-3xl font-bold text-gray-900 dark:text-white mt-1">{children.length}</div>
        </div>
        <div className="bg-white dark:bg-[#1a1a2e] rounded-lg shadow p-4 transition-colors">
          <div className="text-sm text-gray-500 dark:text-gray-400">{t('last7daysObs')}</div>
          <div className="text-3xl font-bold text-gray-900 dark:text-white mt-1">{obs7d}</div>
        </div>
        <div className="bg-white dark:bg-[#1a1a2e] rounded-lg shadow p-4 transition-colors">
          <div className="text-sm text-gray-500 dark:text-gray-400">{t('last7daysProducts')}</div>
          <div className="text-3xl font-bold text-gray-900 dark:text-white mt-1">{media7d}</div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-[#1a1a2e] rounded-lg shadow p-4 mb-4 transition-colors">
        <div>
          <label className="block text-sm text-gray-600 dark:text-gray-300 mb-1">{t('searchChild')}</label>
          <input className="w-full border rounded px-2 py-1.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t('nameSurnamePlaceholder')} />
        </div>
      </div>
