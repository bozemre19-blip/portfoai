...
const domains: DevelopmentDomain[] = ['cognitive','language','social_emotional','fine_motor','gross_motor','self_care'];
const DOMAIN_TR: Record<DevelopmentDomain,string> = {
  cognitive: 'Biliþimsel',
  language: 'Dil',
  social_emotional: 'Sosyal-Duygusal',
  fine_motor: 'Ýnce Motor',
  gross_motor: 'Kaba Motor',
  self_care: 'Öz Bakým',
};
const RISK_TR: Record<RiskLevel,string> = { low: 'Düþük', medium: 'Orta', high: 'Yüksek' };
const DOMAIN_TR: Record<DevelopmentDomain,string> = {
  cognitive: 'Biliþsel',
  language: 'Dil',
  social_emotional: 'Sosyal-Duygusal',
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (ids.length === 0) { setWeekLabels([]); return; }
  ...
  return (
    <>
      <div className="mt-4 bg-white rounded-lg shadow p-4">
        <h2 className="text-lg font-medium text-gray-900 mb-2">Geliþim Alaný Trendi (8 Hafta)</h2>
          ...
          {domains.map((k) => {
            ...
            return (
              <div key={k} className="border rounded p-2">
                <div className="text-sm text-gray-700 mb-1">{DOMAIN_TR[k as DevelopmentDomain] || k}</div>
      ...
      <div className="mt-4 bg-white rounded-lg shadow p-4">
        <h2 className="text-lg font-medium text-gray-900 mb-2">Risk Isý Haritasý (8 Hafta)</h2>
          ...
          {(['low','medium','high'] as RiskLevel[]).map((r) => {
            ...
            return (
              <React.Fragment key={r}>
                <div className="text-gray-700">{RISK_TR[r as RiskLevel]}</div>
                {arr.map((v,i)=> (
                  <div key={i} className="w-6 h-6 rounded-sm" style={{ opacity: Math.max(0.15, v/max), backgroundColor: r==='high'?'#ef4444':r==='medium'?'#f59e0b':'#10b981' }} title={`${r}: ${v}`}></div>
                ))}
              </React.Fragment>
            );
          })}
