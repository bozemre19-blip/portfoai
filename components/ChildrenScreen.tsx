  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-col gap-2">
        {classroomFilter && (
          <div className="flex items-center justify-between bg-blue-50 dark:bg-blue-900/30 border border-blue-100 dark:border-blue-800 text-blue-900 dark:text-blue-100 rounded px-3 py-2 transition-colors">
      ...
      {!loading && !error && filteredChildren.length === 0 && (
        <div className="text-center py-10 bg-white dark:bg-[#1a1a2e] rounded-lg shadow transition-colors">
          <p className="dark:text-gray-300">{t('noChildrenFound')}</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredChildren.map((child, idx) => {
          ...
          return (
            <div key={child.id} className={`bg-white dark:bg-[#1a1a2e] rounded-lg shadow-md p-5 transition-all relative group hover:shadow-xl card-colorful ${cardColors[idx % cardColors.length]}`}>
              <button
              ...
              <div onClick={() => navigate('child-detail', { id: child.id })} className="cursor-pointer">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 font-medium">{t('age')}: {calculateAge(child.dob)}</p>
                    {child.classroom && <p className="text-sm text-gray-600 dark:text-gray-400 font-medium">{t('classroom')}: {child.classroom}</p>}
                  </div>
                </div>
              </div>
            </div>
